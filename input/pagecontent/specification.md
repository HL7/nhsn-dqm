...
1. **Retrieve Measure Bundle:** The dQM Evaluation Engine uses an HTTP GET an NHSNMeasureBundle containing a CRMIShareableMeasure and related resources. The exact Bundle(s) to retrieve is determined by the facility and NHSN during onboarding. After retrieving the Bundle, the dQM evaluation engine parses the contents to determine which resources are needed from the Data Source, then proceeds to step 2.
2. **Query Data Source:** The dQM Evaluation Engine queries the FHIR API of the Data Source (likely a certified EHR in this scenario) using HTTP GET interactions to retrieve the PatientOfInterestList for each Measure retrieved in step 1 (the POI lists in question are determined by the facility and NHSN during onboarding), followed by the resources specified in the Measure(s) for each POI list. The dQM Evaluation Engine then evaluates those data against the Measure(s) and produces MeasureReport resources conforming to DEQMIndividualMeasureReportProfile for each patient that meets the measure criteria, along with a subject-list MeasureReport conforming to the DEQMSubjectListMeasureReport profile containing references to all the individual MeasureReport resources. These MeasureReport resources, along with all the patient data references in them, are then placed in an NHSNMeasureReport bundle for submission to NHSN before proceeding to step 3.
3. **Submit MeasureReport Bundle:** The dQM Evaluation Engine uses an HTTP POST to submit the NHSNMeasureReportBundle to the MeasureReport Recipient. The MeasureReport Recipient validates the Bundle and proceeds to load the data into other NHSN systems (details of such systems are out of scope for this IG).
...
1. **Retrieve Measure Bundle:** The dQM Evaluation Engine uses an HTTP GET an NHSNMeasureBundle containing a CRMIShareableMeasure and related resources. The exact Bundle(s) to retrieve is determined by the facility and NHSN during onboarding. After retrieving the Bundle, the dQM evaluation engine parses the contents to determine which resources are needed from the Data Source, then proceeds to step 2.
2. **Query Data Source:** The dQM Evaluation Engine the Data Source using FHIR or other APIs to retrieve the the resources specified in the Measure(s) for each Patient in the PatientOfInterestList (the POI lists in question are determined by the facility and NHSN during onboarding). The dQM Evaluation Engine then evaluates those data against the Measure(s) and produces MeasureReport resources conforming to DEQMIndividualMeasureReportProfile for each patient that meets the measure criteria, along with a subject-list MeasureReport conforming to the DEQMSubjectListMeasureReport profile containing references to all the individual MeasureReport resources. These MeasureReport resources, along with all the patient data references in them, are then placed in an NHSNMeasureReport bundle for submission to NHSN before proceeding to step 3.
3. **Submit MeasureReport Bundle:** The dQM Evaluation Engine uses an HTTP POST to submit the NHSNMeasureReportBundle to the MeasureReport Recipient. The MeasureReport Recipient validates the Bundle and proceeds to step 4.
